...
public class MergedInventory : InventoryBase {
    ...
    private static BlockPos lastPacketPos = null;
    ...
    public static void MergeRange(IEnumerable<BlockEntity> containers, ICoreClientAPI api) {
        current ??= new(api);
    ...
    public void AddRange(IEnumerable<BlockEntity> containers) {
        int n = count;
        int openIndex = 0;
        foreach (var container in containers) {
            ...
            if (AddPart(part, false)) {
                int delay = 20 + openIndex * 45;
                api.Event.RegisterCallback(_ => {
                    if (current == this && Find(part.Inventory) >= 0) {
                        part.Open();
                    }
                }, delay);
                openIndex++;
        ...
        if (count != n) { 
            UpdateDialog(api);
            int dialogDelay = 160 + openIndex * 45;
            api.Event.RegisterCallback(_ => {
                if (current == this && count > 0) {
                    UpdateDialog(api);
                }
            }, dialogDelay);
        } else if (count > 0) {
    ...
    private bool AddPart(IncludedInventory part, bool open) {
        if (Find(part.Inventory) >= 0) return false;
    ...
    private bool HandleServerPacket(BlockEntityOpenableContainer container, int id) {
        ...
        lastPacketTime = time;
        lastPacketId   = id;
        lastPacketPos  = container.Pos.Copy();
    ...
    private class IncludedInventory {
        ...
        public IncludedInventory(MergedInventory parent,
            ...
            this.api = api;
            Entity = source;
            Inventory = source.FindInventory();
            closeSound = (source as BlockEntityOpenableContainer)?.FindCloseSound();
            openSound = (source as BlockEntityOpenableContainer)?.FindOpenSound();
            Start = start;
            Count = Inventory.Count;
            title = source.FindDialogTitle();
        ...
        public IncludedInventory(MergedInventory parent,
            ...
            closeSound = (Entity as BlockEntityOpenableContainer)?.FindCloseSound();
            openSound = (Entity as BlockEntityOpenableContainer)?.FindOpenSound();
            Start = start;
            Count = Inventory.Count;
        ...
        public void Open() {
            ...
            if (openSound != null) api.Gui.PlaySound(openSound, randomizePitch: true);
        }
        ...
        public void Close() {
            ...
            if (closeSound != null) api.Gui.PlaySound(closeSound, randomizePitch: true);
        }
